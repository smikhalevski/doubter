...
export class Shape<I = any, O = I> {
  ...
  narrow<T>(predicate: (value: any) => value is T, options?: NarrowingOptions | Message): Shape<I, T>;

  narrow(predicate: (value: any) => boolean, options?: NarrowingOptions | Message): this;

  narrow(predicate: (value: any) => boolean, options?: NarrowingOptions | Message): this {
    const checkConfig = createCheckConfig(options, CODE_NARROWING, MESSAGE_NARROWING, predicate);
